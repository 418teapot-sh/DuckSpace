...
| # | 상태 | 처리 |
|---|---|---|
| C-01 남의 이미지 삭제 가능 | ✅ | #76 — 키 접두사로 소유 확인 |
| C-02 메모리 보호가 큰 할당 뒤 | ✅ | #81 |
| C-03 픽셀 상한 업로드 시점 미확인 | ✅ | #81 — 400 `IMAGE_DIMENSION_TOO_LARGE` |
| C-04 종료 대기 < 작업 최악 | ✅ | #92(systemd 150초) + #95(자바 105초 + remove.bg 총 예산) |
| C-05 S3 업로드만 예외 미포장 | ✅ | #82 |
| C-06 로컬 삭제 예외가 배치 중단 | ✅ | #82 |
| C-07 원인 소실 + 도메인 에러코드 | 🔶 **절반** | 아래 참고 |
| C-08 remove.bg 본문 무제한 버퍼링 | ✅ | #82 |
| C-09 `HttpClient` 미종료 | ✅ | #82 |

### C-07 이 절반만 된 이유

원인 예외를 버리던 건 고쳐졌습니다 — 지금은 `new BusinessException(..., e)` 로 넘깁니다(#82).

**남은 건 에러코드입니다.** `LocalImageStorage` · `S3ImageStorage` 는 user · post 도메인도 함께 쓰는 공용 저장소인데, 여전히 `ExhibitionErrorCode.IMAGE_PROCESSING_FAILED` 를 던집니다. 프로필 사진 업로드가 디스크 문제로 실패해도 전시 도메인 에러가 나갑니다.

`StorageErrorCode` 를 따로 만들어야 하는데, `global/` 이 아니라 `exhibition/image/` 에 있는 공용 클래스라 **패키지 위치부터 정리**해야 깔끔합니다. 시연 뒤로 미룹니다.
