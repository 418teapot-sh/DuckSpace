...
### A-07. `/api/popups/**` 가 전체 HTTP 메서드에 열려 있습니다

**위치** `config/SecurityConfig.java:40`

같은 파일 51-59행 javadoc 이 전시를 GET 전용으로 분리한 이유를 이렇게 적어뒀습니다 —
*"경로만으로 열면 아무나 남의 장식장을 지울 수 있게 됩니다."* 그런데 팝업은 메서드 무관
목록에 들어가 있습니다. `/api/home`(38행), `/api/banners`(39행)도 같습니다.

지금은 변경 API 가 전부 `/api/admin/popups` 아래라 **노출된 것은 없습니다.** 다만
**#55(팝업 찜)가 열려 있고**, 찜이 `POST /api/popups/{id}/like` 로 들어오면 그 순간
무인증이 됩니다. 주석이 경고한 바로 그 실패 방식입니다.

**제안** `PUBLIC_GET_ENDPOINTS` 로 옮기기 (`/api/popups/{popupId:[0-9]+}` 형태).
