...
| # | 상태 | 처리 |
|---|---|---|
| E-01 인터럽트 순서로 복구 불가 | ✅ | #74 |
| E-02 남의 이미지 "고정" | ❌ **미해결** | **D-01 소유 검증에 달려 있습니다** — 아래 참고 |
| E-03 큐 거절이 성공한 업로드를 500 으로 | ✅ | #78 |
| E-04 `deleteOrphan` 이 참조된 파일 삭제 | ✅ | #78 |
| E-05 보관함 삭제 409 판단 비원자적 | ✅ | #78 |
| E-06 51번째 장식장부터 접근 불가 | ✅ | #85 |
| E-07 정리 재시도에 backoff 없음 | ✅ | #84 |
| E-08 방치 PENDING 허용 | ✅ | #85 |
| E-09 ~ E-13 나머지 | ✅ | #84 · #85 |
| E-14 두 실행기 종료 순서 | ✅ | #91 — `@DependsOn` |

**14건 중 13건 해결, 1건 미해결.**

### E-02 가 왜 아직 열려 있는가

이 문서가 예측한 그대로입니다 — 본문에 **"D-01 을 미루는 비용이 생각보다 크다"** 고 적어뒀는데, 실제로 그렇게 됐습니다.

#100 이 D-01 의 "우리 저장소인가" 만 넣고 **"누구 것인가" 를 미뤘기 때문에**, 아래 경로가 그대로 열려 있습니다:

1. 공개 피드로 A 의 굿즈 `imageUrl` 을 읽고
2. B 가 그 URL 을 자기 장식장에 배치하면
3. A 의 `DELETE /api/images/{id}` 가 영원히 409 `IMAGE_IN_USE`

**"삭제" 가 삭제가 아니게 됩니다.** 다만 A 의 파일이 지워지지 않을 뿐 **데이터가 손실되지는 않습니다.**

수정 지점은 본문이 적은 대로 **배치 시점의 소유 검증(D-01)** 하나뿐입니다. 정리 쪽 확인을 소유 범위로 좁히는 건 오히려 위험합니다 — 남의 참조를 무시하고 지우게 됩니다.
